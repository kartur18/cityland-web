  ...
  return (
    <div className="fixed inset-0 bg-black/65 z-[10000] flex items-center justify-center backdrop-blur-sm" onClick={(e)=>{if(e.target===e.currentTarget)cerrar()}}>
      <div className="bg-white rounded-3xl max-w-[500px] w-[92%] p-10 text-center relative shadow-[0_32px_80px_rgba(0,0,0,.3)] animate-[popIn_.4s_cubic-bezier(.34,1.56,.64,1)]">
        <button onClick={cerrar} className="absolute top-3.5 right-3.5 bg-[#F4F6FB] border-none w-8 h-8 rounded-full text-base cursor-pointer text-[#4B5680] flex items-center justify-center hover:bg-[#0D1F3C] hover:text-white transition" aria-label="Cerrar">×</button>
        <div className="text-5xl mb-4">✈️</div>
        <h3 className="font-[family-name:var(--font-bebas)] text-4xl text-[#0D1F3C] mb-2 tracking-wide">¡ESPERA! NO TE VAYAS<br/>SIN TU COTIZACIÓN</h3>
        <p className="text-sm text-[#4B5680] leading-relaxed mb-6">Un asesor te envía el mejor precio en <strong>menos de 2 horas</strong>. Es gratis y sin compromiso.</p>
