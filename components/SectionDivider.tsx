...
export default function SectionDivider({ type = "wave", color = "#ffffff", flip = false }: Props) {
  const transform = flip ? "rotate(180deg)" : undefined;

  if (type === "wave") {
    return (
      <div className="w-full overflow-hidden leading-[0] -mb-px" style={{ transform }}>
    ...
    return (
      <div className="w-full overflow-hidden leading-[0] -mb-px" style={{ transform }}>
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-[50px] md:h-[70px]">
          <path d="M600,112.77C268.63,112.77,0,65.52,0,7.23V120H1200V7.23C1200,65.52,931.37,112.77,600,112.77Z" fill={color} />
        </svg>
      </div>
    );
  }
  ...
  return (
    <div className="w-full overflow-hidden leading-[0] -mb-px" style={{ transform }}>
      <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-[50px] md:h-[70px]">
        <path d="M1200 120L0 16.48V0h1200v120z" fill={color} />
      </svg>
    </div>
  );
}
