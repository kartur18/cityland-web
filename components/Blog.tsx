        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((p, index) => (
            <AnimateOnScroll key={p.titulo} stagger={index + 1}>
              <div className="gradient-border bg-white rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,.08)] hover:shadow-[0_12px_40px_rgba(0,0,0,.15)] hover:-translate-y-1 transition group">
                <div className="relative h-[200px] overflow-hidden">
                  <Image src={p.imagen} alt={p.titulo} fill sizes="(max-width:768px) 100vw, 33vw" className="object-cover group-hover:scale-105 transition duration-500" />
