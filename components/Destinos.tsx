      ...
      <div className="max-w-[1200px] mx-auto px-5">
        ...
        {tab === "europa" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {DESTINOS_EUROPA.map((d, index) => (
              <AnimateOnScroll key={d.ciudad} stagger={Math.min(index + 1, 4)}>
                <article className="card-3d gradient-border bg-white rounded-2xl overflow-hidden border border-white/80 transition-all duration-300 shadow-[0_4px_24px_rgba(12,27,58,.06)] hover:-translate-y-2 hover:shadow-[0_20px_48px_rgba(13,31,60,.12)] group">
                  <div className="h-[200px] overflow-hidden relative">
                    <Image src={d.img} alt={d.alt} fill sizes="(max-width:640px) 100vw, (max-width:1024px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-107" />
